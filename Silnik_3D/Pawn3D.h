...
/**
 * @brief Klasa odpowiedzialna za rysowanie i obsługę pionków 3D w grze Ludo.
 *
 * Klasa `Pawn3D` umożliwia:
 * - ładowanie modeli pionków 3D przy pomocy biblioteki Assimp,
 * - rysowanie pionków w domkach i na planszy w odpowiednich kolorach,
 * - podłączenie wskaźników do współrzędnych pionków i ich stanu (czy są na planszy),
 * - ustawienie ścieżek domków graczy i przypisanie nicków do graczy.
 *
 * Przechowuje również wskaźniki na stany pionków i ich pozycje, co pozwala na
 * aktualizowanie pozycji z poziomu zewnętrznego silnika gry (`Engine`).
 */
class Pawn3D {
public:
    Pawn3D();
