...
BitmapHandler* BitmapHandler::handler = nullptr;
...
BitmapHandler::~BitmapHandler() {
    if (handler == this) {
        handler = nullptr;  // Ustawienie handler na nullptr, aby zapobiec dalszemu u¿ywaniu
    }
...
bool BitmapHandler::loadTextures(const std::string& filePath1, const std::string& filePath2, const std::string& filePath3, const std::string& filePath4, const std::string& filePath5, const std::string& filePath6, const std::string& filePath7, const std::string& pionek) {
    ...
    return isTextureLoaded_1 && isTextureLoaded_2 && isTextureLoaded_3 && isTextureLoaded_4 && isTextureLoaded_5 && isTextureLoaded_6 && isTextureLoaded_7 && isTextureLoaded_pionek;
}
...
void BitmapHandler::bindTextureForCube(int steps) {
    /*
    if (!isTextureLoaded_2) {
    ...
    */
    ...
    if (handler == nullptr) {
        std::cerr << "Handler nie jest zainicjowany!" << std::endl;
        return;  // Zakoñcz funkcjê, jeœli handler jest nullptr
    }
    GLuint selectedTexture = 0;

    switch (steps) {
    case 1:
        selectedTexture = handler->texture2;
        std::cerr << "Wybor tekstury 2" << std::endl;
        break;
    case 2:
        selectedTexture = handler->texture3;
        std::cerr << "Wybor tekstury 3" << std::endl;
        break;
    case 3:
        selectedTexture = handler->texture4;
        std::cerr << "Wybor tekstury 4" << std::endl;
        break;
    case 4:
        selectedTexture = handler->texture5;
        std::cerr << "Wybor tekstury 5" << std::endl;
        break;
    case 5:
        selectedTexture = handler->texture6;
        std::cerr << "Wybor tekstury 6" << std::endl;
        break;
    case 6:
        selectedTexture = handler->texture7;
        std::cerr << "Wybor tekstury 7" << std::endl;
        break;
    default:
        std::cerr << "Nieprawidlowa liczba krokow!" << std::endl;
        return;  // Zakoñcz funkcjê, jeœli liczba jest poza zakresem
    }

    if (selectedTexture != 0) {
        std::cerr << "Tekstura zaladowana, bindujemy!" << std::endl;
        glBindTexture(GL_TEXTURE_2D, selectedTexture); // Zastosowanie wybranej tekstury
       
    }
    else {
        std::cerr << "Tekstura nie zostala zaladowana!" << std::endl;
    }
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        std::cerr << "B³¹d podczas bindowania tekstury: " << error << std::endl;
    }
}
/*
